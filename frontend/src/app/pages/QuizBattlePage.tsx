...
export function QuizBattlePage() {
  ...
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'available' | 'leaderboard'>('available');
  ...
  const [battleQuiz, setBattleQuiz] = useState<{ title: string; questions: QuizQuestion[] } | null>(null);
  ...
  const startBattle = async (quiz: Quiz) => {
    try {
